...
from calculator import Calculator, format_result, validate_number
...
def test_subtract_parametrized(
    a: int, b: int, expected: int, calculator: Calculator
) -> None:
...
@pytest.mark.parametrize("value", [1, "1", 1.0, "1.0", -5, "-5", 3.14, "3.14", 0, "0"])
def test_validate_number_parametrized(value):
    """Parametrized test for number validation."""
    assert validate_number(value) == True


@pytest.mark.parametrize("value", ["abc", "", None, [], [1, 2, 3], "hello", "123abc"])
